...
//////////////////////////  ФУНКЦИИ ///////////////////////////////////////
 function reload() {$id=$_GET['id'] ;
  ...
  //  usleep(200000);
  $link="Location: works.php?id=".$id;
    header ($link); 
  //  echo "<script> document.location.href='order.php?id=$id'</script>"; 
  }
  if (!empty($_GET['del']) and $_GET['del'] == 'yes') { $id=$_GET['id'] ;sqlread ("DELETE FROM  works WHERE id='$id'  "); reload();}

  
///////////////////////Обновление данных текущес троки///////////////////////////////////////////////////
function upd_works($field) { $id=$_GET['id'];$data=$_GET[$field];
sqlread("UPDATE works SET    ".$field."= '$data'      WHERE id='$id' ");}
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
if (!empty($_GET['id_board']) ) {   upd_works('id_board');     reload();}
if (!empty($_GET['dates']) ) {      upd_works('dates');        reload();}
if (!empty($_GET['id_board']) ) {   upd_works('id_board');     reload();}
if (!empty($_GET['ser_n']) ) {      upd_works('ser_n');        reload();}
if (!empty($_GET['quantity']) ) {   upd_works('quantity');     reload();}
if (!empty($_GET['user']) ) {       upd_works('user');         reload();}
if (!empty($_GET['type_work']) ) {  upd_works('type_work');    reload();}
if (!empty($_GET['comment']) ) {    upd_works('comment');      reload();}
...
//////////////////////////////////////Добавить новую строку/////////////////////////////////////////////////////////////////////////////////////////////
  ...
  if (!empty($_POST['add']) )  { 
  $userr=$_SESSION['user']; 
  sqlread("INSERT INTO works(user,dates) VALUES ('$userr',NOW())  ");
  reload();
}
...
echo space(180)."<font style='font: 20px apple '  color = 'navy' >СПИСОК РАБОТ ";
echo "</div>";

///////////////////////////////////////////////Шапка////////////////////////////////////////////////////////////////////////////////////////////////////

echo "<div class='head'>";
 echo "<form action='works.php' method='post'  >";
...
$strSQL="SELECT *  FROM works  ORDER BY id DESC  ";
 ...
 $rs = mysql_query($strSQL);
    ...
    while($row = mysql_fetch_array($rs)) {
      echo "<form action='works.php' method='get' id='shap' >";
...
if($_SESSION['user']==21 ) echo "  <td  align='left'  'width=500 bgcolor='LightYellow' id='pole_del' >  <a href='works.php?id=".$id."&del=yes'><img src='images/krest1.png' width='20'   height='20' alt='Пример''></a></td>  "; 
...
?> <td  align='center'  width=40 $bgcolor  id='pole_quant' > <font color='black'  $sizefont1> <b>  <input id = 'pole_quant'  type = 'text' name = 'ser_n' value = '<? echo $ser_n; ?>  ' onchange="this.form.submit()" >     </td> <?
?> <td  align='center'  width=40 $bgcolor  id='pole_id_80' > <font color='black'  $sizefont1> <b>  <input id = 'pole_shipper'  type = 'text' name = 'user' value = '<? echo $user; ?>  ' onchange="this.form.submit()"  >     </td> <?
?> <td  align='center'  width=40 $bgcolor > <font color='black'  $sizefont1> <b>  <input  id = 'pole_defect' type = 'text' name = 'type_work' value = '<? echo $type_work; ?>  ' onchange="this.form.submit()" >     </td> <?
...
?> <td  align='left'  width=40 $bgcolor  > <font color='black'  $sizefont1> <b>  <input id= 'pole_shipper' type = 'text' name = 'shipper' value = '<? echo $comment; ?>  ' onchange="this.form.submit()" >     </td> <?
