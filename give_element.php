...
sqlread("DELETE FROM give_elements_sub  WHERE id  = '$id' ");
sqlread("DELETE FROM give_elements_gen  WHERE id_give_gen  = '$id' ");
...
///// шапка талицы
$name =  array('id ','Наименование','Сер номер'  ,'Номер изд','Кол-во','Статус','','Версия сис','Алгоритм','Примечание') ;
...
foreach ($name as $key => &$nam) {$ln= $long[$key] ; echo "<td align='center'  width='$ln'  height='20'   > <font color='black'   style='font: 12px apple-system'> <p class='gradient'><b> $nam</p></td>";
} echo "</tr> </table> ";
/////end////////
//echo "</form>";
