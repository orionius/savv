...
<head>
 ...
 <meta charset="windows-1251">
  ...
  <link href="bootstrap/css/bootstrap.min.css" rel="stylesheet">  <!--  кнопки стили -->
...
/*
   body {
    background: url(images/<? echo $_SESSION['bacgroun'] ?> ) no-repeat;
...
*/
   ...
   hr {
    ...
    background-color: silver; /* Цвет линии для браузера Firefox и Opera */
...
/*----------------------------------------------------------------БЛОКИ--------------------*/
                ...
                .super_genblock,.genblock,.top_inf,.data,.blok_status,.blok_print, .rash,.blok_manager
 {
   ...
   /* border: 1px double lightgrey;*/
    ...
    position: absolute; /* Абсолютное позиционирование */
    left: 20; /* Положение элемента от правого края */
    top: 140px; /* Положение от верхнего края */
    ...
    margin:auto;
    position: absolute; /* Абсолютное позиционирование */
    left: 1145; /* Положение элемента от правого края */
    top: 142px; /* Положение от верхнего края */
    ...
    margin:auto;
    position: absolute; /* Абсолютное позиционирование */
    left: 1345; /* Положение элемента от правого края */
    top: 142px; /* Положение от верхнего края */
    width: 180px; /* Ширина блока */
    height:400px;    
    padding: 5px; /* Поля вокруг текста */
    ...
    position: absolute; /* Абсолютное позиционирование */
    left: 20; /* Положение элемента от правого края */
    top: 505px; /* Положение от верхнего края */
    width: 1100px; /* Ширина блока */
    height:300px;    
    padding: 5px; /* Поля вокруг текста */
    background: url(images/block_fon.jpg)  no-repeat center center fixed;  background-size: cover;
    ...
    position: absolute; /* Абсолютное позиционирование */
    left: 1145; /* Положение элемента от правого края */
    top: 558px; /* Положение от верхнего края */
...
////////////////////////////////////////////////////////////////////////////////////////////////////////!!!!!!!!!!!!!!!!!!!!!!!!
$user=$_SESSION['user']; 
if($user=='21' || $user =='2' || $user=='23' || $user =='4' ) { 
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
///..............................УСТАНОВИТЬ СТАТУС  АЛГОРИТМ ГОТОВ

if (!empty($_GET['urgently7']) and  strlen($_GET['urgently7']) >2 ) {$id=$_GET['id'] ;
  //echo "<script>alert('У вас ". $_GET['id']." ');</script>";
 $qq=sqlread("UPDATE order_gen SET status_zakaz='11'  WHERE id_order = '$id' "); 
   echo "<script> document.location.href='order.php?id=$id'</script>"; 
 }
...
if (!empty($_GET['urgently8']) and  strlen(   $_GET['urgently8']) >2 ) {$id=$_GET['id'] ;
  $qq=sqlread("UPDATE order_gen SET status_zakaz='12'  WHERE id_order = '$id' "); 
    echo "<script> document.location.href='order.php?id=$id'</script>"; 
  }
  
///..............................УСТАНОВИТЬ СТАТУС Отправлено

if (!empty($_GET['urgently22']) and  strlen(   $_GET['urgently22']) >2 ) {$id=$_GET['id'] ;
  $qq=sqlread("UPDATE order_gen SET status_zakaz='13'  WHERE id_order = '$id' "); 
    echo "<script> document.location.href='order.php?id=$id'</script>"; 
  }
 ///..............................УСТАНОВИТЬ СТАТУС Взято шефом сотрудниками

if (!empty($_GET['urgently20']) and  strlen(   $_GET['urgently20']) >2 ) {$id=$_GET['id'] ;
  $qq=sqlread("UPDATE order_gen SET status_zakaz='14'  WHERE id_order = '$id' "); 
    echo "<script> document.location.href='order.php?id=$id'</script>"; 
  } 
 ///..............................УСТАНОВИТЬ СТАТУС другое

 if (!empty($_GET['urgently21']) and  strlen(   $_GET['urgently21']) >2 ) {$id=$_GET['id'] ;
  $qq=sqlread("UPDATE order_gen SET status_zakaz='15'  WHERE id_order = '$id' "); 
    echo "<script> document.location.href='order.php?id=$id'</script>"; 
  } 
...
if (!empty($_GET['urgently10']) and strlen($_GET['urgently10']) >2 ) {$id=$_GET['id'] ;
  ...
  if (!empty($_GET['urgently11']) and strlen($_GET['urgently11']) >2 ) {$id=$_GET['id'] ;
    $qq=sqlread("UPDATE order_gen SET shipment='10'  WHERE id_order = '$id' "); 
      echo "<script> document.location.href='order.php?id=$id'</script>"; 
    }
...
//echo "<script> document.location.href='order.php?id=".$id."&rashod=''</script>"; 

echo "<script>location.replace('order.php?id=".$id."');</script>"; 
}
...
$max_order++;
 //echo "<script>alert('У вас ". $_GET['id']." ');</script>";
sqlread("INSERT INTO order_gen(n_order,user,date_begin,date,date_end,date_shipment,manager,executor,storekeeper,type_zakaz,shipment)  VALUES ($max_order ,'$user',NOW(),NOW(),NOW() + INTERVAL 20 DAY,NOW() + INTERVAL 30 DAY,'4','3','5','4','8')");
...
/*
  ...
  if (dolarrub==1) {$(".rub:eq("+count+")").val(rubprih);cenarubprih=rubprih;}
  if (dolarrub!=1) {$(".dol:eq("+count+")").val(dolprih);cenadolprih=dolprih;}
  if (dolarrub==1) {$(".rozrub:eq("+count+")").val(rubprih);cenarozrub=rubroz;}
  if (dolarrub!=1) {$(".rozdol:eq("+count+")").val(dolprih);cenarozdol=dolroz;}
...
error:function(html){alertify.error("Внимание !!!  Данные не были сохранены <BR>Возможно отсутствует интернет<BR>Проверьте интернет соединение и введите данные поля снова.");}
  ...
  //  $(".idclient").val(idclient);  записать в поле 
...
?> <input label = ' <? echo $row['company'] ; ?> '   placeholder = ' <? echo $rr['company']; ?> ' list='browsers'  id='idclient' name='idclient' onchange="onchanges_gen();        setTimeout(function(){
...
<datalist id="browsers">
 ...
 $rs = mysql_query("SELECT * FROM client ORDER BY company ASC ");
    if (!empty ($rs) ) {
    while($row = mysql_fetch_array($rs)) {

 $idclient=$row['idclient'];
 $object->idclient = $row['idclient'] ;

 //echo "<option    value = ' $object->idclient ' > ".$object->company ."</option> ";
 echo "<option    value = ' ".$row['idclient'] ." ' > ".$row['company']." </option> ";
    }}
...
?><script>
    ...
    while(count>-1) {
                  ...
                  var price =       document.getElementsByName("price")[count].value;                  
...
echo "<td align='right'  bgcolor='white'  > <font color='black'  > <b> <input type='text' size=1 id='pole_price'  name='price' value=".$price         ." onchange='onchanges();blur(this)'  >   </td>";
...
echo "  <td  align='left'  'width=500 bgcolor='LightYellow' id='pole_del' >  <a href='order.php?id=".$id."&algorithm=".$row['algorithm']."&vers_syst=".$row['vers_syst']."&idbrd_zak=".$idbrd_zak."&quant=1&rashod=yes&add_current=".$id_order_sub."'><img src='images/down.png' width='20'   height='20' alt='Пример''></a></td>  ";
...
<? echo $qqq ;?> <input type="submit" class='btn btn-warning btn-sm' name = "urgently7" value="Алгоритм готов<? echo space(10) ; ?>"><br><p>
<? echo $qqq ;?> <input type="submit" class='btn btn-warning btn-sm' name = "urgently3" value="Прошито<? echo space(20) ; ?>"><br><p>
<? echo $qqq ;?> <input type="submit" class='btn btn-warning btn-sm' name = "urgently22" value="На отправке<? echo space(15) ; ?>"><p>
<? echo $qqq ;?> <input type="submit" class='btn btn-warning btn-sm' name = "urgently8" value="Отправлено в офис<? echo space(2) ; ?>"><p>
<? echo $qqq ;?> <input type="submit" class='btn btn-warning btn-sm' name = "urgently20" value="Взято Шефом,сотруд<? echo space(0) ; ?>"><p>
<? echo $qqq ;?> <input type="submit" class='btn btn-warning btn-sm' name = "urgently21" value="Другое<? echo space(25) ; ?>"><p>
...
?>
<p>
<? echo $qqq ;?> <input type="submit" class='btn btn-success  btn-sm'  name = "print_zakaz" value="Печать заказа&#160&#160&#160&#160&#160&#160&#160&#160&#160&#160&#160"><br><p>
<? echo $qqq ;?> <input type="submit" class='btn btn-success  btn-sm'   name = "packing_list" value="Товарная накладная"><br><p>



<? echo $qqq ;?> <input type="submit" class='btn btn-success  btn-sm'  name = "inform" value="Информировать&#160&#160&#160&#160&#160&#160&#160"><br><p>
<? echo $qqq ;?> <input type="submit" class='btn btn-success  btn-sm'  name = "attire_order" value="Наряд заказ&#160&#160&#160&#160&#160&#160&#160&#160&#160&#160&#160&#160&#160&#160&#160"><br><p>
...
echo space(15);
/////........................список 
$id_board= $_SESSION['id_board']; if(empty($id_board)) {$id_board=1;}
$rr=sqlread("SELECT * FROM cat_brd_prod WHERE id_cat_brd = '$id_board'");
$sql = "SELECT * FROM cat_brd_prod ORDER BY name_product ASC ";
$result_select = mysql_query($sql);
...
//  usleep(200000);
//$link="Location: order.php?id=".$id;  header ($link); 
  echo "<script> document.location.href='order.php?id=$id'</script>"; 
}
?>
