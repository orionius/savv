...
///////////////////////////////////hhh//////////////////////////////////////////////////////
$hashkey= $_GET['qqq'];
$hashkey=   trim($hashkey, "\x00..\x1F");                                                                                                          //          !!!!!!!!----------------!!!!!!!

$h=sqlread("SELECT *  FROM account_gen WHERE hashkey LIKE '$hashkey' ");
...
if($id<10) {  echo "<script> document.location.href=' http://www.savel-ing.ru'</script>";   }
...
echo "<td align='center' id='price'    > ".number_format($price, 2, ',', ' ') ."   </td>";
echo "<td align='center' id='price'    > ".number_format($summa, 2, ',', ' ') ."   </td>";
$nds_str=round( $summa *18/(100+18) ,2 ) ;
...
$nds=round( $summa_gen *18/(100+18) ,2 ) ;
...
echo "<td width = 600 >". space(100) ."в том числе НДС (18%):</td> <td align = right width = 100>" . number_format($sum_nds, 2, ',', ' ')."</td>" ;
...
$numtext=FirstBig(number2string($summa_gen));
